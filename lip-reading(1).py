...
class LipReader:
    ...
    def _predictions_to_text(self, predictions):
        ...
        return "Hello, this is a test of the lip reading system."
    ...
    def text_to_speech(self, text, output_path=None, play_audio=True):
        ...
        """
        try:
            # Initialize TTS with a modern multi-speaker model
            tts = TTS(model_name="tts_models/multilingual/multi-dataset/xtts_v2")
            
            # Generate default output path if none provided
            if output_path is None:
                output_path = str(Path("./generated_speech.wav"))
            
            # Generate speech
            tts.tts_to_file(
                text=text,
                file_path=output_path,
                speaker="p336",  # Default speaker, can be customized
                language="en"
            )
...
def main():
    ...
    # Convert to speech
    audio_path = lip_reader.text_to_speech(
        text,
